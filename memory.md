...
### Resolved (Milestone 3C)
12. **Authoritative Test Reporting and Source-Evidence Hygiene** — Made Vitest (`npx vitest run --reporter=json`) the single authoritative source of truth for test collection, test counts, and inventory generation. Eliminated regex-based static source test count inflation. Redesigned `pnpm test:list` to return Vitest collected test results across 7 required subsystems (`activation`, `audit verification`, `classifier`, `compiler`, `identity`, `identity hardening`, `scanner`). Made `pnpm verify` enforce strict failure gates on count mismatch, missing subsystems, skipped test threshold (0), source artifact hygiene violations, build failures, typecheck failures, and git diff check failures. Removed generated `.d.ts`, `.js`, `.map` build artifacts from `packages/contracts/src/` and added `scripts/guard-source-artifacts.ts` source hygiene guard. Updated scanner evidence priority so authored `.ts`/`.tsx` source is strictly prioritized over `.d.ts` declarations (referencing `packages/contracts/src/activation.ts` instead of `.d.ts`). Trapped child process stderr in `GitPass` (`stdio: ["pipe", "pipe", "pipe"]`) and updated test fixtures to initialize HEAD commits cleanly, eliminating spurious `fatal:` stderr log leaks during clean test runs.
...
- Made Vitest machine-readable JSON output (`npx vitest run --reporter=json`) the single authoritative test execution inventory source for `pnpm test:list` and `pnpm verify`.
...
- **Milestone 3C (Authoritative Test Reporting and Source-Evidence Hygiene)**: Made Vitest the single authoritative source of truth for test collection, corrected `pnpm test:list` to consume Vitest JSON output, updated `pnpm verify` to fail on count or subsystem mismatches, cleaned generated `.d.ts`/`.js`/`.map` artifacts from `packages/contracts/src/`, created `scripts/guard-source-artifacts.ts`, corrected scanner TypeScript evidence priority (`packages/contracts/src/activation.ts`), and suppressed expected Git fixture stderr logs. Branch: `dev/trustworthy-core-v0.1.1`.
